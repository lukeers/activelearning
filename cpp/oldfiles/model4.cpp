...
*/
		...
		void executeRGBModel(string trainLoc,string libLinearLoc,string rgbCSVFile) {
			...
			vector<string> prepareTrainData(vector<string> trainCls,vector<string> testSet) {
				...
				vector<string>  addFoldersAndTestAllALOneFeature(string trainLocation,string tempTestLoc,string libLinearLoc,vector<string> testLabels,bool syn,int negType,string featureType,int execMode,int extraNegPoints) {
					string cmd = "mkdir -p " + trainLocation;
					int ret = system(cmd.c_str());
					string csvFile = "";
						if (featureType == "rgb") {
					...
					for(map<string,vector<string>>::iterator itit=posLabelIndMat.begin();itit != posLabelIndMat.end();++itit) {
						...
						//       }
                                        ...
                                        vector<string> uncertainDataPtns = verifyFullPredictionOneFeature(csvFile,featureType,execMode,extraNegPoints);
				...
				void alBatchModelOneFeature(string dsLoc,string trainLocation,string trainConfFile,string testConfFile,string libLinearLoc,bool syn, string featureType, int execMode, int extraNegPoints ) {
					...
					while(repFlag == false) {
						...
						vector<string> uncertainDataPoints = addFoldersAndTestAllALOneFeature(trainLocation1,tempTestLoc,libLinearLoc,testLabelsAL,syn,negType,featureType,execMode,extraNegPoints);
						...
						vector<string> uncertainDataPoints1 = addFoldersAndTestAllALOneFeature(trainLocation1,tempTestLoc,libLinearLoc,testLabels,syn,negType,featureType,execMode,0);
