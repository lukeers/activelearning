...
*/
		...
		void executeRGBModel(string trainLoc,string libLinearLoc,string rgbCSVFile) {
			...
			int verifyFullPrediction(string csvFile,int type) {
				...
				for(unsigned inst1 = 1;inst1 < tests.size(); inst1++) {
					...
					if(test_n != "") {
						...
						bool found = false;
						for(unsigned inst2 = 0;inst2 < maxIndexes.size(); inst2++) {
							string lbl = labels.at(maxIndexes.at(inst2) - 1);
			...
			int addFoldersAndTestAllAL(string trainLocation,string tempTestLoc,string libLinearLoc,vector<string> testLabels,bool syn,int negType) {
				...
				int predCount = 	verifyFullPrediction(rgbCSVFile,1);
			...
			vector<string> prepareTrainData(vector<string> trainCls,vector<string> testSet) {
				...
				vector<string> verifyFullPredictionOneFeature(string csvFile,string featureType) {
					...
					for(unsigned inst1 = 1;inst1 < tests.size(); inst1++) {
						...
						if(test_n != "") {
							...
							string mchRes = "echo '" + csvFile + " --> "  + test_n + "( " + sample + "  ) predicted as " ;
							for(unsigned inst2 = 0;inst2 < maxIndexes.size(); inst2++) {
								mchRes += " " + labels.at(maxIndexes.at(inst2) - 1);
							}
				...
				vector<string>  addFoldersAndTestAllALOneFeature(string trainLocation,string tempTestLoc,string libLinearLoc,vector<string> testLabels,bool syn,int negType,string featureType) {
					string cmd = "mkdir -p " + trainLocation;
					int ret = system(cmd.c_str());
					string csvFile = "";
						if (featureType == "rgb") {
					...
					for(map<string,vector<string>>::iterator itit=posLabelIndMat.begin();itit != posLabelIndMat.end();++itit) {
						...
						//       }
                                        ...
                                        vector<string> uncertainDataPtns = verifyFullPredictionOneFeature(csvFile,featureType);

					return  uncertainDataPtns;
				}
				...
				void alBatchModelOneFeature(string dsLoc,string trainLocation,string trainConfFile,string testConfFile,string libLinearLoc,bool syn, string featureType) {
					int negType = 3;
					int negTypeDefault = 3;
					cout << "Batch mode started" << endl;
					testAnnotation = {};
					...
					while(repFlag == false) {
						...
						int intervalAL = interval1 / 2;
						vector<string> testSetAL = prepareTestDataAL(trainCls, startPtr + intervalAL, endPtr);
						trainIndexPtrs = addTrainIndexes(trainIndexPtrs, startPtr , startPtr +  intervalAL);         
						...
						vector<string> uncertainDataPoints = addFoldersAndTestAllALOneFeature(trainLocation1,tempTestLoc,libLinearLoc,testLabelsAL,syn,negType,featureType);
						...
						vector<string> uncertainDataPoints1 = addFoldersAndTestAllALOneFeature(trainLocation1,tempTestLoc,libLinearLoc,testLabels,syn,negType,featureType);
				...
				void mlBatchModel(string dsLoc,string trainLocation,string trainConfFile,string testConfFile,string libLinearLoc,bool syn) {
					...
					//int interval  = 100;
					bool repFlag = false;
					int startInterval = 800;
					while(repFlag == false) {
						if (startInterval > 3000) {
							repFlag = true;
						}
						trainCls = trainClsCopy;
						labelIndMat.clear();
				...
				void alEntropyExecuteModel(string dsLoc,string trainLocation,string trainConfFile,string testConfFile,string libLinearLoc,bool syn) {
					...
					string type = "object";
					alBatchModelOneFeature(dsLoc,trainLocation,trainConfFile,testConfFile,libLinearLoc,syn,type);
